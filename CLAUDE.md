...
- **Shared GPU**: the H200s are shared with other users. Start a GPU job on one GPU, and only when `nvidia-smi --id <gpu-uuid> --query-compute-apps=pid --format=csv,noheader` prints nothing for it, then arm `/maindisk/clement/tools/gpu_guard.py <pgid> --log <job-dir>/gpu_guard.log` once the job holds the GPU. The guard counts every GPU process present at arming as ours and stops the job when any new one appears, so keep a job's GPU work in one process. After a yield, a job may relaunch automatically once the GPU has shown no process for an unbroken idle window (`beetle/gpu_resume.py` waits 10 minutes). After any stop that was not a yield, wait for the user to ask before relaunching.
