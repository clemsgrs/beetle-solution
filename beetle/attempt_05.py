"""Prepare, smoke-test, extract, and execute the frozen GenBio-PathFM encoder ablation."""
...
# Relaunches must reuse this Soma run so completed folds are skipped.
RUN_ID = 'attempt-05-genbio-pathfm-20260919'
ENCODER = 'genbio-pathfm'
# Three single-channel 1536-d passes, concatenated per patch token.
FEATURE_DIM = 4608
...
def prepare(base: Path) -> None:
    ...
    config = load_attempt_config(REPO_ROOT / 'configs/attempts/attempt-05.yaml')
    ...
    weights = base / f'huggingface/hub/models--genbio-ai--genbio-pathfm/snapshots/{REVISION}/model.safetensors'
    ...
    write_json(evidence / 'preparation.json', {
        'status': 'completed', 'attempt_id': 'attempt-05', 'roi_count': 124697,
        'slides': len(coords), 'roi_hashes': hashes, 'runtime': runtime,
        'encoder_repository': 'genbio-ai/genbio-pathfm', 'encoder_revision': REVISION,
        'weight_sha256': sha256(weights), 'config_sha256': sha256(base / 'active-config.yaml'),
        'cache_root': str(config.cache.root_dir),
    })
    print('Preparation passed: exact Attempt 01 ROIs/splits, pinned GenBio-PathFM, fresh cache.', flush=True)
...
def smoke(base: Path, config) -> dict:
    ...
    model = Model.from_preset(ENCODER)
    execution = build_execution_options(
        config.encoder, execution=config.execution, encoder_name=ENCODER,
    ...
    if tuple(grid.shape) != (FEATURE_DIM, 32, 32) or grid.dtype != torch.float16 or not torch.isfinite(grid).all():
        raise ValueError('GenBio-PathFM grid failed shape/dtype/numerical checks')
    del model
    gc.collect()
    torch.cuda.empty_cache()
    geometry = compute_dense_geometry(target_size=512, patch_size=16)
    decoder = build_decoder_for_grid(config.decoder.name, config.decoder.params,
                                    geometry=geometry, input_dim=FEATURE_DIM, num_classes=4)
...
def check_revision(base: Path) -> None:
    """slide2vec loads the repo's main ref unpinned, so fail if the hub moved it off REVISION."""
    ref = (base / 'huggingface/hub/models--genbio-ai--genbio-pathfm/refs/main').read_text().strip()
    if ref != REVISION:
        raise ValueError(f'GenBio-PathFM main moved to {ref}; features may not come from {REVISION}')


def load_prepared(base: Path):
    ...
    return prep, replace(load_config(base / 'active-config.yaml'), run_id=RUN_ID)


def extract(base: Path, config) -> dict:
    from beetle.extract import extract_cache

    check_revision(base)
    cache = extract_cache(config, base / 'extraction')
    check_revision(base)
    if (cache['roi_grids'] != 124697 or cache['feature_dim'] != FEATURE_DIM
            or list(cache['grid_shape']) != [32, 32]):
        raise ValueError('Full GenBio-PathFM feature cache has wrong population, channels, or grid')
