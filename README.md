...
| Tag | Config | Dev Dice | Leaderboard overall Dice | Weights |
|---|---|---|---|---|
| `attempt-01` | [attempt-01.yaml](configs/attempts/attempt-01.yaml) | 0.8861 | 0.9063 | [download](https://github.com/clemsgrs/beetle-solution/releases/download/attempt-01/beetle-attempt-01-weights.zip) |
| `attempt-02` | [attempt-02.yaml](configs/attempts/attempt-02.yaml) | 0.8880 | — | [download](https://github.com/clemsgrs/beetle-solution/releases/download/attempt-02/beetle-attempt-02-weights.zip) |
| `attempt-03` | [attempt-03.yaml](configs/attempts/attempt-03.yaml) | 0.8894 | — | [download](https://github.com/clemsgrs/beetle-solution/releases/download/attempt-03/beetle-attempt-03-weights.zip) |
...
For every concluded experiment, submitted or not, score its checkpoints on the unused test folds and add the result and verdict to the [research journal](docs/research-journal.md). Training selects and scores each checkpoint on its tune fold; `python -m beetle score-test --attempt NAME=RUN_DIR ...` scores fold k's checkpoint on organizer fold k from the cached features, resuming where it stopped. `python -m beetle.comparison --split test` builds the paired fold and patient-bootstrap comparison against earlier attempts, and `python -m beetle.release` packages the weights and evidence archives.
