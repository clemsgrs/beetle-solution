...
Attempt 02 was a development-only ablation that reused the verified Attempt 01 features and protocol, changing only the decoder depth from two to four upsampling/convolution blocks. Preflight selected physical batch size 64 with one accumulation step and froze it for all five folds. Its equal-weight five-fold dataset-global mean class Dice on the validation folds that selected its checkpoints was 0.8880192 ± 0.0286550 (sample standard deviation), compared only against Attempt 01's 0.8861255 ± 0.0284992. On the folds held out for evaluation, scored after training, it was 0.8705133 ± 0.0398415 against Attempt 01's 0.8758290 ± 0.0396121. This result did not select or replace an External model.

Attempt 03 was a development-only experiment that reused the verified Attempt 01 features and protocol, changing only the decoder to Soma's `heavy_conv` (5,251,588 parameters): a 1×1 projection from 1,280 to 256 channels; pyramid pooling at 1, 2, 3, and 6 bins with learned projections, concatenated and fused by a 3×3 convolution; two blocks of 2×2 stride-2 transposed convolution, GroupNorm, and ReLU followed by 3×3 convolution, GroupNorm, and ReLU; and a four-class classifier. It kept the 148×148 logit grid and the same resize and crop. Preflight selected physical batch size 64 with one accumulation step. Its equal-weight five-fold dataset-global mean class Dice on the validation folds that selected its checkpoints was 0.8894192 ± 0.0286481 (sample standard deviation), compared against Attempt 01's 0.8861255 ± 0.0284992 and Attempt 02's 0.8880192 ± 0.0286550. On the folds held out for evaluation, scored after training, it was 0.8641969 ± 0.0519641, against 0.8758290 ± 0.0396121 and 0.8705133 ± 0.0398415. This result did not select or replace an External model.
...
Training-batch ROIs were sampled uniformly. The submitted configuration achieved a mean fold-macro dataset-global mean class Dice of 0.8861255 on the five validation folds that selected its checkpoints. Scored after submission on the five folds held out for evaluation, which neither training nor checkpoint selection used, it achieved 0.8758290.
